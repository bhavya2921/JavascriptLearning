...
const AddNewPaletteModal = ({navigation}) => {
  ...
  function onToggle(item) {
    setColorList(prevList => {
      if (!!prevList.find(color => color.colorName === item.colorName)) {
        return prevList.filter(color => color.colorName !== item.colorName);
      }
  ...
  function handleSubmit() {
    if (paletteName === '') {
      Alert.alert('Required', 'Please enter the name of the palette');
      return;
    }
    if (colorList.length < 3) {
      Alert.alert('Required', 'Please select minimum of three color');
      return;
    }
    const newplalette = {paletteName: paletteName, colors: colorList};
    navigation.navigate('home', {newplalette});
  }

  return (
    <View>
      <View>
        <Text style={{padding: 10}}>Name of your color palette</Text>
        <TextInput
          style={myStyle.textInput}
          placeholder={'Add Name'}
          onChangeText={newText => setPaletteName(newText)}
        />
        <FlatList
          style={myStyle.list}
          data={COLORS}
      ...
      <TouchableOpacity onPress={() => handleSubmit()} style={myStyle.buttonS}>
